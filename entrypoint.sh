...
log "🚀 vh-warp 容器启动中..."
log "   INSTANCE_COUNT=${INSTANCE_COUNT} BASE_PORT=${BASE_PORT} LB_PORT=${LB_PORT} LB_STRATEGY=${LB_STRATEGY}"
...
if lb_should_enable; then
    ...
    echo "     策略: ${LB_STRATEGY} (round|random|hash|sticky)"
...
for id in $(instance_id_list); do
    line="$(instance_info_line "$id" 2>/dev/null || echo "instance-${id} ip=n/a")"
    echo "     ${line}"
    echo "     ${line}" >> "$LOG_FILE"
done
echo ""
